	...
	// application. needed because of zmq
	{
		...
		while (true) {
			...
			if (sim_request_obj.type() == panosc::sim_request::STOP) {
				...
				for (size_t i = 0; i < running_simulations[sim_request_obj.hash()].size();
					...
					auto &simulationApplication =
					    running_simulations[sim_request_obj.hash()][i].instance;
			...
			if (lc.is_done() && false) { // in this case re-use the previous results
				...
				sim_request_obj.set_type(
				    panosc::sim_request::QUICK); // forced to be QUICK FIXME
...
#ifdef DEBUG
				std::cout << "------------------------------ Stages\n"
				          << istage << "\t" << panosc::stages.at(istage)
				          << "\tmcpl file:" << mcpl_filename << "\n"
				          << std::endl;
#endif
				...
				if (nJobs > seeds.size()) {
					...
					request->replyBinary(panosc::sim_request_answer_server::to_cameo(
					    panosc::sim_request_answer::ansERROR));
					// throw std::runtime_error("Required number of jobs > the number of
					// predefined seeds");

					continue;
				}
				request->replyBinary(panosc::sim_request_answer_server::to_cameo(
				    panosc::sim_request_answer::ansSTARTING));

				//------------------------------ Start of the submission-retrieval thread
				request_th = std::move(request); // request_th has a higher scope
				std::vector<sim_job_t> &sim_jobs =
				    running_simulations[sim_request_obj.hash()];
				panosc::simHash_t hash = sim_request_obj.hash();

				thread.reset(new std::thread([&server, &publisher, &running_simulations,
				                              &sim_jobs, app_name, lc, istage, &request_th,
				                              args, seeds, nJobs, hash]() {
					// std::unique_ptr<std::thread> merging_thread;

					//---------- function to merge the jobs
					/* starts a merge job and waits for it to end and then publishes the
					 * result */
					auto mergeJobs = [&server, &publisher, &lc,
					                  &sim_jobs](bool finished) {
						std::string filenames =
						    (lc.output_dir_merge().parent_path() / "merge_list.txt");
						std::cout << "[INFO] Filename: " << filenames << std::endl;
						std::ofstream merge_filelist(filenames);
						if (merge_filelist.is_open() == false) {
							throw std::runtime_error(
							    "Error opening file for list of merging MCPL "
							    "files");
						}
						...
						for (size_t iJob = 0; iJob < sim_jobs.size(); iJob++) {
							...
							if (sim_job.state == cameo::application::SUCCESS) {
								// filenames += (lc.output_dir(iJob) /
								// "sDETECTOR.mcpl.gz"); filenames += ";";
								merge_filelist << (lc.output_dir(iJob) /
								                   "sDETECTOR.mcpl.gz")
								                      .string()
								               << "\n";
							}
					...
					auto submitOneJob = [&server, &sim_jobs, app_name, &seeds, &lc,
					                     args]() {
					...
					for (size_t fJob = 0; fJob < nJobs; fJob++) {
						...
						if (sim_jobs.size() <
						    nJobs) { // iJob is the index of the submitted
